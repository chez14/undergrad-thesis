...
\section{Analisis Pemilihan Framework & Library}
...
\section{Kebutuhan Tambahan}
    Kebutuhan tambahan datang dari lingkungan pengembangan yang nantinya akan
    dibuat untuk mengimplementasi solusi.
    
    Kebutuhan tambahan yang pertama datang dari penggunaan subsistem front-end
    yang harus di-\textit{build} terlebih dahulu sebelum peramban dapat menjalankan
    kode pada subsistem tersebut. Dengan begitu, perlu diadakannya sebuah sistem
    \textit{delivery} yang otomatis dan terkontrol untuk melakukan proses \textit{build}
    tersebut.
    
    Proses tersebut akan ditangani oleh sistem CI/CD dari repositori tempat aplikasi
    disimpan: GitLab. Karena repositori aplikasi disimpan pada GitLab, maka fitur
    CI/CD dapat digunakan. Sistem ini nantinya akan dirancang untuk melakukan
    \textit{build} untuk sistem front-end.
    
    Kebutuhan tambahan kedua adalah dengan otomasi test. Karena konsekuensi aplikasi
    yang dibuat cukup besar, oleh karena itu test yang otomatis harus ditambahkan untuk
    mencegah kesalahan yang cukup fatal sebelum aplikasi dapat dikirimkan ke wilayah
    produksi.
